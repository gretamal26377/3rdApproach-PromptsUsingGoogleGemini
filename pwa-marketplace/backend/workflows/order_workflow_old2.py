...
class OrderWorkflow:
    ...
    async def aggregate_item_statuses(self):
        ...
        """
        try:
            while self.current_status != "customer_accepted":
                item_statuses = []
                ...
                if new_aggregate_status != self.current_status:
                    ...
                    # Update DB using Activity
                    # False-positive error: Temporal Team & Community suggest ignoring it and adding type: # type: ignore[arg-type]
                    await workflow.execute_activity(
                        order_activities.update_order_status_in_db,
                        self.order_id,
                        self.current_status,
                        # This is mandatory Temporal timeout for Activity execution. schedule_to_close_timeout is total allowed time for Activity to complete
                        # (including schedulling delay + execution time + completion report time). If exceeded, Activity is retried according to retry policy
            ...
            # Handle cancellation of this loop when workflow finishes and it runs automatically when workflow ends
            pass
