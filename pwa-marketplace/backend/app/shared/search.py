...
def search_products():
    ...
    try:
        ...
        # 1. Perform an initial search to get all matching product listings
        ...
        # 2. For each unique base product, find the listing with the lowest price
        aggregated_results = []
        # Sequence unpacking where 2nd value is not needed (_), named this way following Python convention
        for base_product_id, _ in facet_distribution['base_product_service_id'].items():
            # Perform a filtered and sorted search to find the best-priced item
            # for this specific base product
            product_specific_results = index.search(
                search_term,
                {
                    'filter': f'base_product_service_id = {base_product_id}',
                    'sort': ['price:asc'],
                    'limit': 1 # We only need the top result (the one with the lowest price)
                }
            )

            if product_specific_results.hits:
                best_hit = product_specific_results.hits[0]
                # 3. Structure the lightweight JSON for the frontend
                aggregated_results.append({
                    'base_product_service_id': best_hit['base_product_service_id'],
                    'name': best_hit['product_name'],
                    'thumbnail_url': best_hit.get('product_pic_path'),
